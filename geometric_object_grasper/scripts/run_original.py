...
def test_c_shape():
    ...
    robot_hand = BarrettHand(urdf_file='../assets/robot_hands/barrett/bh_282.urdf', contacts_file_name='contacts_230')

    for i in range(4):
        init_angle = default_params['bhand_angles'][i]
        robot_hand.update_links(joint_values=[0.0, init_angle, init_angle, init_angle])
        visuals = robot_hand.get_visual_map(frames=False)
...
def test_floating_gripper(args):
    ...
    from gog.environment_original import Environment
    ...
    for i in range(args.n_episodes):
        ...
        while True:
            ...
            # Sample candidates
            candidates, candidates_rand_points = policy.get_candidates(state, plot=False)
            ...
            # opt_id = np.argmin(scores)
                ...
                # pose = np.matmul(opt_preshape['pose'], np.linalg.inv(robot_hand.ref_frame))
                ...
                o3d.visualization.draw_geometries([pcd_contacts, state, base_frame])
