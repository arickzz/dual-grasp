...
class Environment:
    ...
    def __init__(self,
        ...
        self.pxl_size = 0.005
        self.bounds = np.array([[-0.5, 0.5], [-0.5, 0.5], [0.01, 1]])  # workspace limits
    ...
    def add_objects(self, episode=0):
        ...
        obj_paths = self.obj_files[episode]
        self.obj_curr_name = os.path.basename(obj_paths)
        ...
        body_id = pybullet_utils.load_obj(obj_path=obj_paths, scaling=1, position=np.array([0.0, 0.0, 0.0]),
                                            orientation=Quaternion().as_vector("xyzw"))
        ...
        pos=[0,0,-float(np.array(p.getAABB(body_id)[0][2]))]
        if size[0] > size[1]:
    ...
    def get_obs(self):
        ...
        Get observation.
        """
        obs = {'color': [], 'depth': [], 'seg': [], 'full_state': [], 'name': []}

        obs['name'] = self.obj_curr_name
